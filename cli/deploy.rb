...
begin
  ...
  ARGV.each do |arg|
    ...
    when /^--(\w+)=(.*)$/
      ...
      puts "Usage: deploy.rb [--config=<config_file>] [--node=<node_name>] [--ssh=<ssh_credentials>] [--local] [--root] [--deploy_ops=op1,op2,...] [--param1=value1] [--param2=value2] ..."
      exit 1
    end
  end
  ...
  if local
    # Local execution
    if deploy_ops.nil?
      puts "Error: --deploy_ops is required when using --local."
      puts "Usage: deploy.rb --local --deploy_ops=op1,op2,... [--param1=value1] [--param2=value2] ..."
      exit 1
    end

    deploy_ops.each do |op_file|
    ...
    if node_name
      ...
      # If using --ssh, then --deploy_ops is required
      if deploy_ops.nil?
        puts "Error: --deploy_ops is required when using --ssh."
        puts "Usage: deploy.rb --ssh=<ssh_credentials> --deploy_ops=op1,op2,... [--param1=value1] [--param2=value2] ..."
        exit 1
      end
      node[:deploy_ops] = deploy_ops
    ...
    ops_list = node[:deploy_ops]
    if ops_list.nil? || !ops_list.is_a?(Array) || ops_list.empty?
      raise "No .op files specified in the node's :deploy_ops list."
    end
